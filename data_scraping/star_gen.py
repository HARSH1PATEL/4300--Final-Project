...
api_key = '' #taken out for security
...
for place in data[2620:]:
  ...
  try:
    req = requests.get('https://maps.googleapis.com/maps/api/place/textsearch/json?query='+search1+'&key='+api_key, timeout=5)
    time.sleep(1)
  except:
    try:
      req = requests.get('https://maps.googleapis.com/maps/api/place/textsearch/json?query='+search2+'&key='+api_key, timeout=5)
      time.sleep(1)
    except:
      try:
        req = requests.get('https://maps.googleapis.com/maps/api/place/textsearch/json?query='+search3+'&key='+api_key, timeout=5)
        time.sleep(1)
      except:
        f.write('3.0'+'\n')
